...
def resync_linkedin(payload: dict) -> dict:
    ...
    if not profile_meta.linkedin_url(pdir):
        raise RoleScoutError("add and save a LinkedIn URL before resyncing")
    started = PROFILE_MANAGER.start(
        person,
...
def save_profile(payload: dict, files: dict | None = None) -> dict:
    ...
    if isinstance(raw_removals, str):
        try:
            ...
            raise RoleScoutError("remove_materials must be a JSON list") from exc
    if not isinstance(raw_removals, list):
        raise RoleScoutError("remove_materials must be a list")
    ...
    if unknown:
        raise RoleScoutError("not removable material file(s): " + ", ".join(sorted(unknown)))
    ...
    for filename, data in (value for value in files.values() if isinstance(value, tuple)):
        ...
        if clean in uploads:
            raise RoleScoutError(f"duplicate upload filename: {clean}")
        if len(data) > 15 * 1024 * 1024:
            raise RoleScoutError(f"file too large: {clean} (15MB max)")
        uploads[clean] = data
...
def update_project(code: str, payload: dict) -> dict:
    ...
    if view.returncode != 0:
        raise RoleScoutError("preferences saved, but job visibility refresh failed: " +
...
def _parse_multipart(handler: BaseHTTPRequestHandler) -> tuple[dict, dict]:
    ...
    if length < 0 or length > MAX_UPLOAD_BODY:
        raise RoleScoutError(f"upload body exceeds {MAX_UPLOAD_BODY} bytes")
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _trusted_request(self, *, allow_bootstrap: bool = False) -> bool:
        ...
        for header in ("Origin", "Referer"):
            ...
            if origin_port not in {None, expected_port}:
                return False
        supplied = self.headers.get("X-RoleScout-Token", "")
        if allow_bootstrap:
    ...
    def do_GET(self):  # noqa: N802
        ...
        if url.path in ("/", "/index.html"):
            ...
            body = UI_PATH.read_text(encoding="utf-8").replace(
                "__ROLESCOUT_SESSION_TOKEN__", self.server.session_token).encode("utf-8")
    ...
    def do_POST(self):  # noqa: N802
        ...
        if url.path == "/api/profile/save":
            ...
            except (RoleScoutError, ValueError, OSError) as e:
                return self._json({"error": str(e)}, 400)
...
def serve(port: int = 8787) -> ThreadingHTTPServer:
    try:
        ...
        raise RoleScoutError(
            ...
            "stop that RoleScout instance or choose `rolescout web --port <other-port>`"
        ) from exc
