"""`rolescout init` — person/project intake wrapping scripts/new_project.py.
...
def _save_person_meta(person: str, args) -> None:
    ...
    if url:
        try:
            ...
            raise RoleScoutError(str(e)) from e
    if fields:
...
def main(args) -> int:
    ...
    if not sys.stdin.isatty():
        print("usage: rolescout init --person <slug> --focus <slug> --locations '…' "
    ...
    root = repo_root()
    print("RoleScout setup — a *person* holds the profile; a *project* is one search\n"
    ...
    print(f"  resume files: copy them into profiles/{person}/ "
          "(pdf/docx/md; or upload via `rolescout web` → Profile)")
