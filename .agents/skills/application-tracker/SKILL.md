...
3. Validate then write: `python3 scripts/upsert_rows.py tracker rows.json` (runs `validate_tracker_rows.py` with job-list linkage and transition checks, upserts into SQLite on `application_id`, verifies read-after-write, regenerates the CSV mirrors and xlsx pipeline view).
