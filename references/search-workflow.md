...
**Division of labor (non-negotiable):** deterministic scripts own baseline source execution, provider enumeration, JD capture, URL normalization, validation, and persistence. LLM work belongs before or after capture: market-map reasoning, company-universe expansion when requested, fit scoring, grouping, resume targeting, and interview synthesis. Scripts own repeatable mechanics: `scripts/resolve_company_sources.py` gives an official-first source skeleton, `scripts/deterministic_search.py` runs provider-first baseline discovery and JD extraction, `scripts/build_location_search_urls.py` expands registry location-format hints into candidate search URLs, `scripts/fetch_url.py` handles public fetch/cache summaries, `scripts/normalize_job_url.py` canonicalizes URL/job_id fields, `scripts/location_eligibility.py` classifies exact-city vs same-metro location ambiguity, `scripts/analyze_search_plan.py` gates plan readiness before capture, `scripts/merge_research_parts.py` merges legacy shard logs, `scripts/generate_coverage_audit.py` writes deterministic coverage audit scaffolding, `scripts/analyze_search_coverage.py` classifies complete/partial/blocked capture, `scripts/probe_linkedin_jobs.py` records optional runner-owned LinkedIn Jobs observations, `scripts/persist_job_rows.py` normalizes/validates/upserts rows, `scripts/validate_research_artifacts.py` verifies search artifacts, `scripts/validate_linkedin_review.py` verifies LinkedIn prep artifacts, and `scripts/validate_interview_prep.py` verifies interview prep artifacts. Do not write one-off Python for these common tasks; if a deterministic adapter is missing, report the capability gap. Do not read external helper skills; use RoleNavi skills/references plus the deterministic scripts. A script must never decide *which company to search* beyond declared project targets or existing company-universe artifacts.
...
5. **LinkedIn Jobs — optional supplemental discovery, run LAST when used, with a strict login contract.** Baseline search must finish through non-login deterministic sources first. LinkedIn can add semantic discovery for title families per-company scans miss, but signed-out or authwalled LinkedIn must create a supplemental follow-up rather than blocking baseline completion. The runner may record the navigation observation through `python scripts/probe_linkedin_jobs.py <project>` so this does not depend on the Codex agent's own sandbox. The contract, identical across skill/runner/prompts:
   ...
   - **Partial progress then handoff**: when blockage is observed, finish everything else, write all five artifacts, persist validated rows, note the optional pending pass in coverage-audit.md, and report the supplemental follow-up. The *user* performs any login; the agent never enters credentials. The rerun is **incremental**: LinkedIn pass only, appended to existing artifacts.
