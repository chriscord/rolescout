...
각 installer는 명령을 실행한 위치에 `./rolenavi`를 만듭니다. launcher가 내부 Python 환경을 관리하고 로컬 web UI를 열기 때문에 사용자가 `.venv`를 활성화할 필요가 없습니다. 다른 위치를 쓰려면 installer 실행 전에 `ROLENAVI_INSTALL_DIR`를 설정하세요. 같은 명령을 다시 실행하면 변경사항이 없는 정상 RoleNavi checkout을 안전하게 갱신하고 설치를 재개합니다. 관련 없는 디렉터리나 tracked change는 덮어쓰지 않습니다.

아래 Unix 명령은 `rolenavi` 디렉터리 안에서 `./start`를 실행하는 기준입니다. Windows에서는 `.\start.cmd`를 사용하세요.
...
`./start`는 `.venv`를 활성화하지 않아도 설치본의 `rolenavi web`과 같은 동작을 실행합니다. 브라우저에서 `http://127.0.0.1:8787`로 자동 연결되며, UI는 loopback 전용이고 외부에 호스팅되지 않습니다.
...
| 명령 | 예상 결과 |
|---|---|
| `./start init --person you --focus ai-product --locations "San Francisco"` | 프로필/프로젝트 쌍 생성 또는 활성화. `--companies`, `--role`, `--level`, `--comp-range`, `--negatives`로 프로젝트 선호사항 지정 가능. |
| `./start run profile-intake --person you` | resume/material 및 허용된 LinkedIn current-source content로 `profiles/<person>/candidate-profile.md`, `profiles/<person>/evidence-map.md` 생성 또는 갱신. |
| `./start run opportunity-plan` | 모델 사용이 허용된 target preference로 범위가 제한된 typed company universe를 선택적으로 생성. |
| `./start run search` | 결정적 provider-first discovery를 실행하고 direct posting URL/JD snapshot을 수집해 raw Jobs store와 UI-visible Jobs view를 작성. 기본적으로 scoring은 실행하지 않음. |
| `./start run score` | UI에 보이는 모든 Jobs row를 runner가 만든 압축 batch로 평가한 뒤 가중 점수와 `fit_score`/`priority`를 결정적으로 다시 계산해 기록. |
| `./start run prep` | focused 포지션 기준 strategy, resume, LinkedIn, interview 준비 동시 실행. |
| `./start run prep-strategy` | 그룹화된 application strategy와 priority plan만 생성. |
| `./start run prep-resume` | focused job group별 targeted resume draft 생성. |
| `./start run prep-linkedin` | LinkedIn current → to-be 개선안 생성. |
| `./start run prep-interview` | focused 포지션별 interview pack과 story bank 생성. |
| `./start run story-bank` | 이력서 근거 기반의 공용 story bank를 독립적으로 다시 생성. |
| `./start run apply` | focused 포지션별 application instruction과 tracker row 생성. 자동 제출 없음. |
| `./start export --public` / `--private` | 민감도에 따라 분리된 export와 revision manifest를 명시적으로 생성. |
| `./start privacy audit` | 비공개 내용을 출력하지 않고 로컬 runtime/telemetry 흔적을 보고. |
| `./start clean --runtime` | 보존 데이터 dry-run manifest 출력. 삭제하려면 `--apply` 추가. |
| `./start delete-person --person <slug>` | 프로필/프로젝트 삭제 dry-run manifest 출력. 삭제하려면 `--apply` 추가. |

프로젝트 전환은 `./start init --activate <code>`, 1회성 프로젝트 지정은 `--project <code>` 사용.
...
수정 가능한 파일은 `./start doctor` 또는 라이브 Codex 실행 시 `~/.rolenavi/model-profiles.json`에 생성됨. 해당 파일을 직접 수정하거나 다른 JSON 파일 지정 가능:
