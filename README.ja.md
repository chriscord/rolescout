...
<p align="center">
  <img src="assets/demo.gif" alt="RoleScoutワークフローのデモ" width="960">
</p>
...
RoleScoutはホスト型バックエンドを運営せず、利用者のキャリアデータをRoleScoutのサーバーに収集しません。ソースファイル、保存領域、生成資料は端末内に残ります。ただし、ライブ合成の実行時には、認証済みCLIを通じて最小化されたワークフローパケットが選択したモデルプロバイダーへ送信されます。既定はCodexです。Codex合成は使い捨てのstagingディレクトリで読み取り専用sandboxとして開始され、shell/unified-exec/apps/web search、会話履歴は無効、プロセス環境はallowlistに限定されます。RoleScoutは最初のライブ実行前にプロバイダー通知を表示し、各ワークフローが使用するデータ分類を報告します。連絡先、応募状況、報酬履歴、就労許可、LinkedIn URL、無関係な非公開メモは既定でモデルプロンプトから除外され、目標報酬はモデル利用可能な検索希望条件です。
...
外部CLI接続（開発者向け。RoleScoutは任意のCLIのsandboxを検証できません）:
...
外部プロバイダーのファイルシステム・ツール分離を確認した後に限り、`ROLESCOUT_ENABLE_UNSANDBOXED_CLI=1`を設定してください。プロセスはallowlist環境の使い捨てstagingディレクトリから開始されます。
...
3. **計画 → 収集 → 評価 → 確定。** `opportunity-plan`は検証済みcompany universeを作成する任意の範囲限定モデル段階です。`search`はそのuniverse（またはseed-onlyモードで宣言したseed）から決定的に収集し、URL/JDの正規化と保存を行います。`score`は圧縮した求人batchを意味評価に送り、決定的なfinalize段階で重みを適用してスコアを書き込みます。星を付けて関心ポジションを**focused**として登録します。
...
| `rolescout run profile-intake --person you` | 履歴書/資料と受け入れ済みLinkedIn current-source contentから`profiles/<person>/candidate-profile.md`と`profiles/<person>/evidence-map.md`を作成または更新。 |
| `rolescout run opportunity-plan` | モデル利用可能なtarget preferenceから範囲限定のtyped company universeを任意で作成。 |
| `rolescout run search` | 決定的なprovider-first discoveryを実行し、direct posting URL/JD snapshotを収集してraw Jobs storeとUI-visible Jobs viewを構築。既定ではscoreを実行しない。 |
| `rolescout run score` | UIに表示中のJobs rowをrunner作成の圧縮batchで評価し、重み付きスコアと`fit_score`/`priority`を決定的に再計算して書き込む。 |
...
| `rolescout run story-bank` | 履歴書根拠に基づく共有story bankを単独で再生成。 |
| `rolescout run apply` | focusedポジション別のapplication instructionとtracker rowを生成。自動提出なし。 |
| `rolescout export --public` / `--private` | 機密度で分離したexportとrevision manifestを明示的に作成。 |
| `rolescout privacy audit` | 非公開内容を表示せず、ローカルruntime/telemetryの残存状況を報告。 |
| `rolescout clean --runtime` | 保持データのdry-run manifestを表示。削除には`--apply`を追加。 |
| `rolescout delete-person --person <slug>` | プロフィール/プロジェクト削除のdry-run manifestを表示。削除には`--apply`を追加。 |
...
| 機密度分離ストア | 公開求人情報は`data/public-opportunities.db`、非公開pipeline状態は`private/pipeline.db`に保存。exportも明示実行し、分離を維持。 |
