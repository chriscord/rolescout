...
各installerはコマンドを実行した場所へ`./rolenavi`を作成します。launcherが内部Python環境を管理してローカルweb UIを開くため、利用者が`.venv`を有効化する必要はありません。別の場所を使う場合はinstaller実行前に`ROLENAVI_INSTALL_DIR`を設定してください。同じコマンドを再実行すると、変更のない正規のRoleNavi checkoutを安全に更新してインストールを再開します。無関係なディレクトリやtracked changeは上書きしません。

以下のUnixコマンドは`rolenavi`ディレクトリ内で`./start`を使う前提です。Windowsでは`.\start.cmd`を使用してください。
...
`./start`は`.venv`を有効化せずに、インストール済み環境の`rolenavi web`と同じ動作を実行します。ブラウザーで`http://127.0.0.1:8787`を自動表示し、UIはloopback専用でホスト公開されません。
...
| コマンド | 期待される結果 |
|---|---|
| `./start init --person you --focus ai-product --locations "San Francisco"` | プロフィール/プロジェクトの組み合わせを作成または有効化。`--companies`、`--role`、`--level`、`--comp-range`、`--negatives`でプロジェクト希望条件を指定可能。 |
| `./start run profile-intake --person you` | 履歴書/資料と受け入れ済みLinkedIn current-source contentから`profiles/<person>/candidate-profile.md`と`profiles/<person>/evidence-map.md`を作成または更新。 |
| `./start run opportunity-plan` | モデル利用可能なtarget preferenceから範囲限定のtyped company universeを任意で作成。 |
| `./start run search` | 決定的なprovider-first discoveryを実行し、direct posting URL/JD snapshotを収集してraw Jobs storeとUI-visible Jobs viewを構築。既定ではscoreを実行しない。 |
| `./start run score` | UIに表示中のJobs rowをrunner作成の圧縮batchで評価し、重み付きスコアと`fit_score`/`priority`を決定的に再計算して書き込む。 |
| `./start run prep` | focusedポジション向けにstrategy、resume、LinkedIn、interview準備をまとめて実行。 |
| `./start run prep-strategy` | グループ化されたapplication strategyとpriority planのみ生成。 |
| `./start run prep-resume` | focused job group別のtargeted resume draftを生成。 |
| `./start run prep-linkedin` | LinkedIn current → to-be改善案を生成。 |
| `./start run prep-interview` | focusedポジション別のinterview packとstory bankを生成。 |
| `./start run story-bank` | 履歴書根拠に基づく共有story bankを単独で再生成。 |
| `./start run apply` | focusedポジション別のapplication instructionとtracker rowを生成。自動提出なし。 |
| `./start export --public` / `--private` | 機密度で分離したexportとrevision manifestを明示的に作成。 |
| `./start privacy audit` | 非公開内容を表示せず、ローカルruntime/telemetryの残存状況を報告。 |
| `./start clean --runtime` | 保持データのdry-run manifestを表示。削除には`--apply`を追加。 |
| `./start delete-person --person <slug>` | プロフィール/プロジェクト削除のdry-run manifestを表示。削除には`--apply`を追加。 |

プロジェクト切り替えは`./start init --activate <code>`、1回だけのプロジェクト指定は`--project <code>`を使用。
...
編集可能なファイルは、`./start doctor`またはライブCodex実行時に`~/.rolenavi/model-profiles.json`へ作成。直接編集、または別のJSONファイル指定が可能:
