...
This module intentionally implements RoleNavi-native search mechanics rather
...
class HttpClient:
    def __init__(self, timeout_s: int = DEFAULT_TIMEOUT_S) -> None:
        ...
        self.user_agent = (
            "RoleNavi/1.0 deterministic job search "
...
def _snapshot(
    ...
    classified = jd_text_cleaner.classify_requirements(jd_text)
    return {
        "schema": "rolenavi-jd-snapshot-v1",
        "job_id": row["job_id"],
        "company": candidate.company,
        "title": candidate.title,
...
def _lifecycle_manifest(results: list[DiscoveryResult], run_id: str) -> dict[str, Any]:
    ...
    return {"schema": "rolenavi-job-lifecycle-manifest-v1",
...
def run_search(
    ...
    *,
    ...
    """Run deterministic search and write RoleNavi-compatible artifacts."""
    ...
    _write_json(details_dir / "source-tasks.json", {
        "schema": "rolenavi-source-tasks-v1",
        "generated_at": _now_utc(),
        "tasks": task_records,
    })
    _write_json(details_dir / "candidates.json", {
        "schema": "rolenavi-deterministic-candidates-v1",
        "generated_at": _now_utc(),
        "kept_rows": rows,
    ...
    summary = {
        "schema": "rolenavi-deterministic-search-summary-v1",
        "generated_at": _now_utc(),
        "project": project.name,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(description="Run deterministic RoleNavi search.")
