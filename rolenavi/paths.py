...
class RoleNaviError(RuntimeError):
...
def repo_root() -> Path:
    env = os.environ.get("ROLENAVI_ROOT")
    if env:
        p = Path(env).resolve()
        if not all((p / m).is_dir() for m in _MARKERS):
            raise RoleNaviError(
                f"ROLENAVI_ROOT={env} is not a RoleNavi repo (missing {'/'.join(_MARKERS)})")
        return p
    found = _walk_up(Path.cwd()) or _walk_up(Path(__file__).parent)
    if not found:
        raise RoleNaviError(
            "Not inside a RoleNavi repo. cd into your RoleNavi clone "
            "(the folder containing scripts/ and references/), or set ROLENAVI_ROOT.")
    return found


def home_dir() -> Path:
    env = os.environ.get("ROLENAVI_HOME")
    p = Path(env).expanduser() if env else Path.home() / ".rolenavi"
