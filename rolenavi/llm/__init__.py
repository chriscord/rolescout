"""rolenavi.llm — provider isolation layer (dev-plan §0).
...
Backends (all optional; deps live ONLY in their own module):
  codex   OpenAI Codex CLI, user's ChatGPT subscription via `codex login`
          (`codex.py` - no API key, RoleNavi never sees credentials)
  cli     any local agent CLI command you already authenticated yourself
          (`external_cli.py` - configured by ROLENAVI_LLM_CMD)
  mock    canned envelopes; zero deps, zero accounts, zero network (`mock.py`)

Selection (ROLENAVI_PROVIDER overrides; otherwise automatic):
  LLM_MOCK=1 / --mock             -> mock
  ROLENAVI_PROVIDER=codex|cli    -> that backend (errors if unusable)
  ROLENAVI_LLM_CMD set           -> cli
...
def provider_choice(force_mock: bool = False) -> str:
    ...
    if force_mock or os.environ.get("LLM_MOCK") == "1":
        return "mock"
    explicit = os.environ.get("ROLENAVI_PROVIDER", "").strip().lower()
    if explicit:
        if explicit not in _ALIASES:
            raise RoleNaviError(
                f"ROLENAVI_PROVIDER={explicit!r} unknown (use codex, cli, or mock)")
        return _ALIASES[explicit]
    if os.environ.get("ROLENAVI_LLM_CMD"):
        return "cli"
