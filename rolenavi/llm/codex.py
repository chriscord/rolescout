...
then bills usage to their subscription. RoleNavi never sees credentials — it
only shells out to the locally installed `codex` binary.

Install: `npm install -g @openai/codex` (or `brew install codex`), then
`codex login`. Select this provider with `ROLENAVI_PROVIDER=codex` (it is also
...
remain pinned by RoleNavi workflow profiles.
...
class CodexProvider:
    ...
    def __init__(self) -> None:
        ...
        if not exe:
            raise RoleNaviError(
                "ROLENAVI_PROVIDER=codex but the `codex` CLI is not on PATH — "
    ...
    def _strip_pinned_model_args(args: list[str]) -> list[str]:
        """Remove env-supplied args that would override RoleNavi's run contract."""
        out: list[str] = []
        i = 0
        while i < len(args):
            arg = args[i]
    ...
    def _exec_command(self, workflow: str | None = None,
        ...
        extra += profile_args
        # Prompt is passed on stdin. Passing the full RoleNavi prompt as argv
    ...
    def _exec(self, prompt: str, workflow: str | None = None) -> tuple[str, list[dict]]:
        ...
        if r.returncode != 0:
            ...
            raise RoleNaviError(f"codex exec failed (rc={r.returncode}): "
                                 f"{detail[:400]}")
        events = []
    ...
    def run(self, workflow: str, context: dict, on_progress=None,
        ...
        last_error: RoleNaviError | None = None
        ...
        raise last_error or RoleNaviError("codex exec failed before starting")
    ...
    def _run_once(self, workflow: str, prompt: str, profile: dict,
        ...
        try:
            ...
            while True:
                ...
                if elapsed > TIMEOUT_S:
                    proc.kill()
                    raise RoleNaviError(f"codex exec exceeded CODEX_TIMEOUT_S={TIMEOUT_S}s")
                try:
                ...
                if isinstance(item, BaseException):
                    raise RoleNaviError(f"codex stdout read failed: {item}")
                line = item
                line = line.rstrip("\n")
                if not line.strip():
                    continue
        ...
        if proc.returncode != 0:
            ...
            raise RoleNaviError(f"codex exec failed (rc={proc.returncode}): "
                                 f"{detail[:1000]}")
