...
from ..paths import RoleNaviError, home_dir
...
def model_profile_path() -> Path:
    override = os.environ.get("ROLENAVI_MODEL_PROFILES")
...
def load_profiles() -> tuple[dict[str, Any], Path]:
    ...
    try:
        ...
        raise RoleNaviError(f"model profile config is not valid JSON: {path} ({e})") from e
    except OSError as e:
        raise RoleNaviError(f"cannot read model profile config: {path} ({e})") from e
    if not isinstance(custom, dict):
        raise RoleNaviError(f"model profile config must be a JSON object: {path}")
...
def _profile_for(provider: str, workflow: str | None, *,
    ...
    if not isinstance(provider_settings, dict):
        raise RoleNaviError(f"model profile config is missing {provider} settings: {path}")

    default = provider_settings.get("default", {})
    workflows = provider_settings.get("workflows", {})
    if not isinstance(default, dict) or not isinstance(workflows, dict):
        raise RoleNaviError(f"model profile config has invalid {provider} settings: {path}")
    ...
    effort = effort.lower()
    if not model:
        raise RoleNaviError(f"model profile for workflow {workflow!r} is missing model: {path}")
...
def codex_profile_for(workflow: str | None) -> dict[str, str]:
    return _profile_for("codex", workflow, model_env="ROLENAVI_CODEX_MODEL",
                        effort_env="ROLENAVI_CODEX_EFFORT", validate_effort=True)


def external_cli_profile_for(workflow: str | None) -> dict[str, str]:
    return _profile_for("external_cli", workflow, model_env="ROLENAVI_LLM_MODEL",
                        effort_env="ROLENAVI_LLM_EFFORT", validate_effort=False)
